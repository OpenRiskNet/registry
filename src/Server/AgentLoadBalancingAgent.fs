...
type AgentLoadBalancingAgent<'state, 'message, 'agent when 'agent :> IAgent<'state, 'message>>(agents: IAgent<'state, 'message> seq, mergeStatesFn: 'state -> 'state -> 'state, cancelToken: CancellationToken) =
    ...
    interface IAgentLoadBalancingAgent<'state, 'message> with
        ...
        member this.ReadonlyState =
            agents
            |> Seq.map (fun agent -> agent.ReadonlyState)
            |> Seq.reduce mergeStatesFn
