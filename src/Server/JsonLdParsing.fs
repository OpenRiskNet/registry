...
let fixOrnJsonLdContext (OpenApiDereferenced openapiString) =
    let rec recursiveRenameContext (token: Chiron.Json) =
        ...
        | Chiron.Array items ->
            items
            |> List.map recursiveRenameContext
            |> Chiron.Array
...
let loadJsonLdIntoTripleStore (OpenApiFixedContextEntry openapiString): Result<TripleStore, string> =
    result {
...
let createSparqlQuery (queryString: string): Result<SparqlQuery, string> =
    ...
    try
        ...
        Ok query
    with :? VDS.RDF.Parsing.RdfParseException as ex -> Error(sprintf "Error parsing the Sparql query: %s" ex.Message)
...
let runQuery (store: TripleStore) (query: SparqlQuery) =
    ...
    try
        ...
        | :? SparqlResultSet as r ->
            ...
            | VDS.RDF.Query.SparqlResultsType.VariableBindings ->
                ...
                |> Seq.map (fun result ->
                    query.Variables
                    |> Seq.map (fun var ->
                        if result.HasValue(var.Name) then result.[var.Name].ToString()
                        else "")
                    |> Seq.toList)
                |> Seq.toList
                |> fun results ->
                    Orn.Registry.Shared.BindingResult
                        { Variables =
                              query.Variables
        ...
        | _ -> Error "Item was not of expected type"
    with :? VDS.RDF.Parsing.RdfParseException as ex -> Error(sprintf "Error parsing the Sparql query: %s" ex.Message)
