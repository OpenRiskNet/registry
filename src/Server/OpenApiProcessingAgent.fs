...
type OpenApiProcessingAgent(feedbackAgent: Feedback.IFeedbackAgent, servicesAgent: IOpenApiServicesAgent, cancelToken: CancellationToken) =
    ...
    let rec agentFunction (feedbackAgent: Feedback.IFeedbackAgent) (servicesAgent: IOpenApiServicesAgent)
            ...
            | IndexNewUrl((OpenApiUrl url) as openApiUrl, maybeAlreadyProcessedEntry, reindexInterval) ->
                ...
                try
                    let! result = asyncResult {
                                      ...
                                      let! openapistring = SafeAsyncHttp.AsyncHttpTextResult
                                      ...
                                      let hash =
                                          hasher.ComputeHash
                                              (System.Text.Encoding.UTF8.GetBytes(openapistring: string): byte [])

                                      match processingInfo with
                                      | { Status = Reindexing oldServiceInfo;
                                          OpenApiRetrievalInformation = Some { Hash = oldHash } } as previousInfo when System.Collections.StructuralComparisons.StructuralEqualityComparer.Equals
                                                                                                                           (hash,
                                                                                                                            oldHash) ->
                                      ...
                                      | _ ->
                                          ...
                                          updateProcessingInfo
                                              { processingInfo with OpenApiRetrievalInformation = Some(retrievalInfo) }
                                          printfn "Downloading worked, processing..."

                                          let retrievedAt = System.DateTimeOffset.UtcNow

                                          let! description, openapi = openapistring
                                                                      |> OpenApiRaw
                                                                      |> TransformOpenApiToV3Dereferenced retrievedAt
                                                                             (openApiUrl)
                                                                      |> Result.teeError
                                                                          (fun err ->
                                                                          feedbackAgent.Post
                                                                              (OpenApiParsingFailed(openApiUrl, err)))
                                          let! jsonld = fixOrnJsonLdContext openapi
                                                        |> Result.teeError
                                                            (fun err ->
                                                            feedbackAgent.Post(JsonLdParsingError(openApiUrl, err)))
                                          updateProcessingInfo
                                              { processingInfo with
                                                    DereferencedOpenApi =
                                                        Some(OpenApiFixedContextEntry(jsonld.Unwrap())) }
                                          let! tripleStore = loadJsonLdIntoTripleStore jsonld
                                                             |> Result.teeError
                                                                 (fun err ->
                                                                 feedbackAgent.Post(JsonLdParsingError(openApiUrl, err)))
